...
// Pre-set this password in the file to behave like a simple static-site "env" value.
// NOTE: in a client-only static app, this password is visible in the source code.
const PRESET_ADMIN_PASSWORD = 'your-admin-password';
...
// Prompt to set a new admin password (used if none exists and no preset password is configured)
async function promptSetPassword(){
  if(PRESET_ADMIN_PASSWORD){
    // If a preset password exists, do not prompt to create a new one.
    return false;
  }
...
async function promptAdminLogin(){
  try{
    ...
    if(PRESET_ADMIN_PASSWORD){
      existingHash = await hashPassword(PRESET_ADMIN_PASSWORD);
    }
    if(!existingHash){
